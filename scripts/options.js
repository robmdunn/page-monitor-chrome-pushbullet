...
function exportPagesList(a) {
  a && getAllPages(function (b) {
    ...
    for (var d in b) {
      ...
      c.push('            <!--PageMonitorAdvancedPageData=' + f + '-->\n')
    }
    c.push('</DL><p>');
    a(c.join(''))
  })
...
function importPagesList(a) {
  for (var b = RegExp('(<[aA][^<>]+>[^<>]+</[aA]>)(?:\\s*<!--PageMonitorAdvancedPageData=({.*?})-->)?', 'g'), c, e = 0; c = b.exec(a, b.lastIndex); ) {
    var d = $(c[1]),
    f = d.attr('HREF') || '',
...
function initializeGlobalControls() {
  ...
  initializePushbulletEnable();
  initializePushbulletKey();
  initializePushbulletDevices();
  initializeExporter();
  initializeImporter();
...
function initializeColorPicker() {
  ...
  b = getSetting(SETTINGS.badge_color) || [0,
  180,
  0,
  255],
  b = '#' + a(b[0]) + a(b[1]) + a(b[2]);
...
function initializeIntervalSliders() {
  ...
  b.val(timeAbsoluteToLog(a)).change(function () {
    var a = 60000 * timeLogToAbsolute(parseFloat($(this).val()));
...
function initializeNotificationsTimeout() {
  var a = getSetting(SETTINGS.notifications_timeout) / 1000 || 30;
  $('#notifications_timeout input').val(a).change(function () {
    var a = 1000 * parseFloat($(this).val()),
    a = 60000 < a ? chrome.i18n.getMessage('until_closed')  : describeTime(a);
...
function initializePushbulletEnable() {
  $('#pushbullet_enable input:checkbox').change(function () {
    var a = $(this).prop('checked');
    setSetting(SETTINGS.pushbullet_enable, a)
  }).prop('checked', getSetting(SETTINGS.pushbullet_enable))
}
function initializePushbulletKey() {
  $('#pushbullet_key input:text').change (function () {
  	var a = $(this).prop('value');
  	setSetting(SETTINGS.pushbullet_key, a);
    initializePushbulletDevices();
  }).prop('value', getSetting(SETTINGS.pushbullet_key));
}
function initializePushbulletDevices() {
  var devices = getSetting(SETTINGS.pushbullet_devices) || [];
  PushBullet.APIKey = getSetting(SETTINGS.pushbullet_key);
  PushBullet.devices(function(err, res) {
    $("#pushbullet_devices").empty()
    if(err) {
      console.log(err);
    } else {
      $.each(res.devices, function(index, device) {
        $("#pushbullet_devices").append(
          $("<div>").append(
      ...
      $('#pushbullet_devices input:checkbox').each( function(cb_index, cb_element) {
        $(cb_element).change( function () {
          devices = [];
          $("#pushbullet_devices input:checkbox").filter(':checked').each(function(index, element) {
            devices.push($(element).val());       
          });
          setSetting(SETTINGS.pushbullet_devices, devices);
        });
      });
    }
  });
...
function sortPagesInplace(a, b) {
  if ('date added' != b) {
    ...
    a.sort(function (a, d) {
      if ('name' == b) a = a.name,
      d = d.name;
       else if ('check interval' == b) a = a.check_interval || c,
      d = d.check_interval || c;
       else if ('last check' == b) a = - a.last_check,
      d = - d.last_check;
       else if ('last change' == b) a = - a.last_changed || 0,
      d = - d.last_changed || 0;
       else if ('url' == b) a = a.url,
      d = d.url;
...
function addPageToTable(a) {
  ...
  var e = timeAbsoluteToLog(e / 60000),
  g = $('.page_interval', b);
