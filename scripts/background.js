...
DEFAULT_CHECK_INTERVAL = 10800000,
RESCHEDULE_DELAY = 900000,
MINIMUM_CHECK_SPACING = 1000,
BROWSER_ICON = 'img/browser_icon.png',
EPSILON = 500,
WATCHDOG_INTERVAL = 900000,
WATCHDOG_TOLERANCE = 120000;
(function () {
  var b = 0,
  a = null,
  e = !1,
  ...
  triggerDesktopNotification = function () {
    if (getSetting(SETTINGS.notifications_enabled) && !(0 < chrome.extension.getViews({
      ...
      var b = getSetting(SETTINGS.notifications_timeout) || 30000;
      if (window.webkitNotifications && webkitNotifications.createHTMLNotification) {
      	a = window.webkitNotifications.createHTMLNotification('notification.htm'),
        a.show();	
        ...
        if (0 != b.length) {
          ...
          if(getSetting(SETTINGS.pushbullet_enable) && getSetting(SETTINGS.pushbullet_devices).length > 0) {
          	PushBullet.APIKey = getSetting(SETTINGS.pushbullet_key);
          	devices = getSetting(SETTINGS.pushbullet_devices);
          	$.each(devices, function(index, device_iden) {
              $.each(b, function(index, elem) {
                PushBullet.push("link", device_iden, null, {title: c.title+" - "+elem.name, url: elem.url});
              });
            });
          }
        }
      }),
      e || (chrome.notifications.onButtonClicked.addListener(function (b, a) {
        var c = d[a];
      ...
      60000 >= b && setTimeout(hideDesktopNotification, b)
    }
  };
...
(function () {
  ...
  check = function (a, b, c) {
    $.ajax({
      ...
      complete: function (f) {
        f && 200 <= f.status && 300 > f.status ? actualCheck(a, b, c)  : (actualCheck(a, b, c), console.log('Network appears down (' + (f && f.status) + '). Checking anyway.'))
      }
    })
  };
  watchdog = function () {
...
function importVersionOnePages(b) {
  ...
  $.each(getSetting('pages_to_check') || {
  }, function (b, d) {
    a.push({
      url: b,
      name: d.name,
...
function bringUpToDate(b, a) {
  initializeStorage(function () {
    ...
    1 > b ? (setSetting(SETTINGS.badge_color, [
      0,
      180,
      0,
      255
    ]), setSetting(SETTINGS.check_interval, DEFAULT_CHECK_INTERVAL), setSetting(SETTINGS.sound_alert, null), setSetting(SETTINGS.notifications_enabled, !1), setSetting(SETTINGS.notifications_timeout, 30000), setSetting(SETTINGS.animations_disabled, !1), setSetting(SETTINGS.sort_by, 'date added'), setSetting(SETTINGS.view_all_action, 'original'), e())  : 2 > b ? (setSetting(SETTINGS.view_all_action, 'original'), delSetting('last_check'), importVersionOnePages(e))  : 3 > b ? (setSetting(SETTINGS.check_interval, getSetting('timeout') || DEFAULT_CHECK_INTERVAL), setSetting(SETTINGS.view_all_action, 'original'), delSetting('timeout'), importVersionTwoPages(e))  : e()
  })
};
