var SETTINGS = {
  ...
  pushbullet_devices: 'pushbullet_devices'
},
...
function describeTime(a) {
  ...
  e || (f = chrome.i18n.getMessage('second'), g = chrome.i18n.getMessage('seconds', a.toString()), e += ' ' + (1 == a ? f : g));
...
function sqlResultToArray(a) {
  for (var b = [
  ], d = 0; d < a.rows.length; d++) b.push(a.rows.item(d));
  return b
}
function getPage(a, b) {
  b && executeSql('SELECT * FROM pages WHERE url = ?', [
    a
  ], function (a) {
...
function getAllPageURLs(a) {
  a && executeSql('SELECT url FROM pages', [
  ], function (b) {
    for (var d = [
    ], c = 0; c < b.rows.length; c++) d.push(b.rows.item(c).url);
    a(d)
  })
}
function getAllPages(a) {
  a && executeSql('SELECT * FROM pages', [
  ], function (b) {
    a(sqlResultToArray(b))
  })
}
function getAllUpdatedPages(a) {
  a && executeSql('SELECT * FROM pages WHERE updated = 1', [
  ], function (b) {
    a(sqlResultToArray(b))
  })
}
...
function removePage(a, b) {
  executeSql('DELETE FROM pages WHERE url = ?', [
    a
  ], null, function () {
    BG.scheduleCheck();
    (b || $.noop) ()
