...
function hasTags(a) {
  for (var b = 0; b < a.length; b++) if ('number' == typeof a[b]) return !0;
  return !1
}
  ...
  eachOpcode(a, function (a, n, m, p, k) {
    if ('replace' != a[0]) g.push(a);
      ...
      if (e || a) {
        var l = n,
        q = m;
        if (e) for (e = n; e < m && !b[e].match(/^</); e++) l++;
        for (e = m - 1; e >= n &&
        !b[e].match(/^</); e--) q--;
        var s = p,
        t = k;
        if (a) for (e = p; e < k && !c[e].match(/^</); e++) s++;
    ...
    if ('replace' != a[0]) g.push(a);
      ...
      if (e != l) {
        ...
        l > q ? (l = n + e, e = p + e, m != l ? a = [
          ...
          e,
          k
        ]), m = l)  : (l = m - e, k -= e, n != l ? g.push(['delete',
        n,
        l,
        0,
        0])  : p != k && g.push(['insert',
      ...
      for (k = 0; k < m - n; k++) {
        ...
        [
          s[0],
          calculateHtmlDiff($(b[e]).html(), $(c[l]).html(), h),
          '</' + s[1] + '>'
        ].join(''), s = hashToken(q, h), b[e] = c[l] = q, d[e] = f[l] = s, g.push(['equal',
        e,
        e + 1,
        l,
        l + 1]))  : (g.push(['delete',
...
function clusterRuns(a, b, c, d, f) {
  ...
  }); h = h.concat(g); return h = h.concat(r)
}
function assembleHtmlDiff(a, b, c) {
  ...
  eachOpcode(a, function (a, g, r, n, m) {
    ...
    f = a[0]
  }); 'delete' == f ? d.push(DEL_END)  : 'insert' == f && d.push(INS_END); return d
}
...
function generateControls(a) {
  ...
  a = '<div id="chrome_page_monitor_ext_orig_link">                     <a class="pm_original" href="%url%"                        title="%title%">%original%</a>                     <br />                     <a class="pm_textize" href="#">%textize%</a>                     <br />                     <a class="pm_hide" href="#">%hide%</a>                   </div>'.replace('%url%', a).replace('%title%', c).replace('%original%', d).replace('%textize%', f).replace('%hide%', b ? g : r);
  a = $(a);
  b || $('del').hide();
...
function getInlineStyles(a) {
  a = a.match(/<style[^>]*>(.*?)<\/style>/gi);
  var b = [
  ];
  if (a) for (var c = 0; c < a.length; c++) b.push(a[c].replace(/<\/?style[^>]*>/gi, ''));
  return b.join('\n')
}
function getReferencedStyles(a) {
  return (a = a.match(/<link[^>]*>/gi)) ? $(a.join('')).filter('[rel="stylesheet"]')  : $([])
}
